...
int32_t main(int32_t argc, char **argv) {
  ...
  while (running) {
    ...
    int64_t process_ticks = new_cutime + new_cstime - old_cutime - old_cstime;
    int64_t new_ms = new_tv.tv_sec * 1000 + new_tv.tv_nsec / 1000000;
    float period_sec = (new_ms - old_ms) / 1000.0;
    // printf("old %ld new %ld  period_sec %.2f\n", old_ms, new_ms, period_sec);
    float cpu_usage = process_ticks / (period_sec * clockTicks);
    printf("cpu_usage %.2f %\n", cpu_usage * 100);

    std::string mem_cmd{"cat /proc/" + std::to_string(process_id) + "/statm"};
    std::string mem_ack{};
    ret = ReadCmd(mem_cmd.data(), mem_ack, 2);
    if (ret == 0) {
      if (!ack.empty()) {
